...
function wpwisebones_has_sidebar(): bool {
	return in_array( wpwisebones_get_layout(), array( 'left-sidebar', 'right-sidebar' ), true )
...
function wpwisebones_posted_on() {
	...
	echo wp_kses_post( '<span class="posted-on me-3"><i class="bi bi-calendar3 me-1"></i>' . $time . '</span>' );
}

function wpwisebones_posted_by() {
	echo wp_kses_post( '<span class="byline me-3"><i class="bi bi-person me-1"></i><a href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">' . esc_html( get_the_author() ) . '</a></span>' );
}
...
function wpwisebones_entry_footer() {
	...
	if ( $cats ) {
		echo '<span class="cat-links me-3"><i class="bi bi-folder me-1"></i>' . wp_kses_post( $cats ) . '</span>'; // phpcs:ignore WordPress.Security.EscapeOutput
	}   if ( $tags ) {
		echo '<span class="tags-links me-3"><i class="bi bi-tags me-1"></i>' . wp_kses_post( $tags ) . '</span>';  // phpcs:ignore WordPress.Security.EscapeOutput
...
function wpwisebones_social_share() {
	...
	$networks = array(
		'twitter'  => array( 'https://twitter.com/intent/tweet?url=%s&text=%s', 'Twitter / X', 'bi-twitter-x' ),
		'facebook' => array( 'https://www.facebook.com/sharer/sharer.php?u=%s', 'Facebook', 'bi-facebook' ),
		'linkedin' => array( 'https://www.linkedin.com/sharing/share-offsite/?url=%s', 'LinkedIn', 'bi-linkedin' ),
		'whatsapp' => array( 'https://api.whatsapp.com/send?text=%s%%20%s', 'WhatsApp', 'bi-whatsapp' ),
	);
