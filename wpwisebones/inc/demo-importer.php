	...
	if ( isset( $_POST['wpwisebones_demo_action'] ) && check_admin_referer( 'wpwisebones_demo_action_nonce' ) ) {
		...
		if ( 'install' === $action ) {
			...
			if ( is_wp_error( $result ) ) {
				...
				$notices[] = array( 'success', __( 'Demo content installed! Visit your site to see it in action.', 'wpwisebones' ) );
				$installed = true;
			}
		} elseif ( 'reset' === $action ) {
			wpwisebones_demo_reset();
			$notices[] = array( 'success', __( 'Demo content removed. Your site is clean again.', 'wpwisebones' ) );
			$installed = false;
		}
	}
...
function wpwisebones_demo_create_images(): array {
	...
	$ids     = array();
	$colours = array(
		array( 13, 110, 253 ),   // Bootstrap primary blue
		array( 25, 135, 84 ),    // Bootstrap success green
		array( 220, 53, 69 ),    // Bootstrap danger red
		array( 255, 193, 7 ),    // Bootstrap warning yellow
		array( 13, 202, 240 ),   // Bootstrap info cyan
		array( 111, 66, 193 ),   // Bootstrap purple
		array( 52, 58, 64 ),     // Bootstrap dark
	);
	$labels  = array(
		'Hero',
		'Design',
	...
	foreach ( $colours as $i => $rgb ) {
		...
		$attach_id = wp_insert_attachment(
			array(
				...
				'meta_input'     => array( '_wpwisebones_demo' => 1 ),
			),
			$filepath
		);
...
function wpwisebones_demo_create_category(): int {
	...
	$result = wp_insert_term( 'WPWiseBones Demo', 'category', array( 'slug' => 'wpb-demo' ) );
...
function wpwisebones_demo_setup_widgets(): void {
	...
	// Footer col 2
	$sidebars['footer-2'] = array( 'recent-posts-3' );

	// Footer col 3
	$sidebars['footer-3'] = array( 'categories-3' );
