...
function wpwisebones_customize_register( WP_Customize_Manager $wp_customize ) {
	...
	$color_settings = array(
		'wpwisebones_color_primary'   => array( __( 'Primary Colour', 'wpwisebones' ), '#0d6efd' ),
		'wpwisebones_color_secondary' => array( __( 'Secondary Colour', 'wpwisebones' ), '#6c757d' ),
		'wpwisebones_color_accent'    => array( __( 'Accent Colour', 'wpwisebones' ), '#6610f2' ),
		'wpwisebones_color_header_bg' => array( __( 'Header Background', 'wpwisebones' ), '#ffffff' ),
		'wpwisebones_color_footer_bg' => array( __( 'Footer Background', 'wpwisebones' ), '#1a1a2e' ),
	);
...
function wpwisebones_enqueue_google_fonts() {
	...
	if ( $fonts_to_load ) {
		...
		wp_enqueue_style(
			...
			array(),
			null // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion -- null intentional; prevents version query string on external Google Fonts URL.
		);
	}
}
...
function wpwisebones_customizer_css() {
	...
	$raw_size  = get_theme_mod( 'wpwisebones_base_font_size', 16 );
	$font_size = max( 12, min( 24, absint( $raw_size ? $raw_size : 16 ) ) );
