...
$wpwisebones_unique_id = esc_attr( uniqid( 'search-form-' ) );
?>
<form role="search" method="get" class="search-form d-flex" action="<?php echo esc_url( home_url( '/' ) ); ?>">
	<label for="<?php echo esc_attr( $wpwisebones_unique_id ); ?>" class="visually-hidden">
		<?php esc_html_e( 'Search for:', 'wpwisebones' ); ?>
	</label>
	<input
		type="search"
		id="<?php echo esc_attr( $wpwisebones_unique_id ); ?>"
