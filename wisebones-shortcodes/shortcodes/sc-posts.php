...
function wpbs_sc_posts( array $atts ): string {
	...
	$query_args = array(
		...
		'order'          => in_array( strtoupper( $a['order'] ), array( 'ASC', 'DESC' ), true ) ? $a['order'] : 'DESC',
		'no_found_rows'  => true,
	);
	...
	while ( $posts->have_posts() ) {
		...
		if ( 'true' === $a['show_thumb'] && has_post_thumbnail() ) {
			...
			$html .= get_the_post_thumbnail( null, $a['image_size'], array( 'class' => 'card-img-top' ) );
			$html .= '</a>';
		}
		...
		if ( 'true' === $a['show_date'] || 'true' === $a['show_author'] ) {
			...
			if ( 'true' === $a['show_date'] ) {
				$html .= '<i class="bi bi-calendar3 me-1"></i>' . esc_html( get_the_date() ) . ' ';
			}
			if ( 'true' === $a['show_author'] ) {
				$html .= '<i class="bi bi-person me-1"></i>' . esc_html( get_the_author() );
			}
			$html .= '</p>';
		}
