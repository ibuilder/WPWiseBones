...
function wpbs_sc_row( array $atts, ?string $content = null ): string {
	...
	$classes = array( 'row', 'wpb-columns', 'g-' . absint( $a['gutter'] ) );
	if ( $a['align'] ) {
		$classes[] = 'justify-content-' . sanitize_html_class( $a['align'] );
	}
	if ( $a['valign'] ) {
		$classes[] = 'align-items-' . sanitize_html_class( $a['valign'] );
	}
	if ( $a['class'] ) {
...
function wpbs_sc_col( array $atts, ?string $content = null ): string {
	...
	if ( $a['sm'] ) {
		$classes[] = 'col-sm-' . absint( $a['sm'] );
	}
	if ( $a['md'] ) {
		$classes[] = 'col-md-' . absint( $a['md'] );
	}
	if ( $a['lg'] ) {
		$classes[] = 'col-lg-' . absint( $a['lg'] );
	}
	if ( $a['xl'] ) {
		$classes[] = 'col-xl-' . absint( $a['xl'] );
	}
	if ( $a['align'] ) {
		$classes[] = 'text-' . sanitize_html_class( $a['align'] );
	}
	if ( $a['class'] ) {
		$classes[] = esc_attr( $a['class'] );
	}
