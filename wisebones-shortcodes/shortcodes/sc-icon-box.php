...
function wpbs_sc_icon_box( array $atts, ?string $content = null ): string {
	...
	$classes = array( 'wpb-icon-box', 'text-' . sanitize_html_class( $a['align'] ) );
	if ( $a['class'] ) {
		$classes[] = esc_attr( $a['class'] );
	}
	if ( $a['bg'] ) {
		$classes[] = 'bg-' . sanitize_html_class( $a['bg'] ) . ' p-4 rounded';
	}
	...
	$html .= $icon_html;
	if ( $a['title'] ) {
		$html .= '<h5 class="mt-2 mb-1">' . esc_html( $a['title'] ) . '</h5>';
	}
	if ( $content ) {
		$html .= '<p class="text-muted">' . wp_kses_post( do_shortcode( $content ) ) . '</p>';
	}
	$html .= $a['url'] ? '</a>' : '';
	$html .= '</div>';
