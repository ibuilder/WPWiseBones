...
function wpbs_sc_button( array $atts, ?string $content = null ): string {
	...
	$classes = array( 'btn', 'btn-' . sanitize_html_class( $a['style'] ), 'wpb-btn-shortcode' );
	if ( $a['size'] ) {
		$classes[] = 'btn-' . sanitize_html_class( $a['size'] );
	}
	if ( $a['class'] ) {
	...
	$rel    = esc_attr( $a['rel'] ? $a['rel'] : ( '_blank' === $a['target'] ? 'noopener noreferrer' : '' ) );
