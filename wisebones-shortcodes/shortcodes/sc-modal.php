...
function wpbs_sc_modal( array $atts, ?string $content = null ): string {
	...
	$modal_id = $a['id'] ? $a['id'] : 'wpbModal' . $count;

	$dialog_cls = 'modal-dialog';
	if ( $a['size'] ) {
		$dialog_cls .= ' modal-' . sanitize_html_class( $a['size'] );
	}
	if ( 'true' === $a['centered'] ) {
		$dialog_cls .= ' modal-dialog-centered';
	}
	if ( 'true' === $a['scrollable'] ) {
		$dialog_cls .= ' modal-dialog-scrollable';
	}
