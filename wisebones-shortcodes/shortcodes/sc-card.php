...
function wpbs_sc_card( array $atts, ?string $content = null ): string {
	...
	$classes = array( 'card', 'wpb-card-shortcode', 'h-100' );
	if ( $a['shadow'] ) {
		$classes[] = 'shadow-' . sanitize_html_class( $a['shadow'] );
	}
	if ( $a['border'] ) {
		$classes[] = 'border-' . sanitize_html_class( $a['border'] );
	}
	if ( $a['text'] ) {
		$classes[] = 'text-' . sanitize_html_class( $a['text'] );
	}
	if ( $a['class'] ) {
	...
	if ( $a['title'] ) {
		$html .= '<h5 class="card-title">' . esc_html( $a['title'] ) . '</h5>';
	}
	if ( $a['subtitle'] ) {
		$html .= '<h6 class="card-subtitle mb-2 text-muted">' . esc_html( $a['subtitle'] ) . '</h6>';
	}
	if ( $content ) {
		$html .= '<p class="card-text">' . wp_kses_post( do_shortcode( $content ) ) . '</p>';
	}
	if ( $a['btn_text'] ) {
